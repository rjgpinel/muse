...
class PickEnv(BaseEnv):
    def __init__(self, num_distractors=0, **kwargs):
        ...
        assert self.num_distractors < 3, "Only up to 2 distractors are allowed"
        self.cubes_color = [
            (0.74, 0.13, 0.10),
            (0, 0.48, 0.36),
            (0.93, 0.86, 0.16),
        ]
        self.initial_cubes_pos = [[0, 0, -2], [0, 0, -2], [0, 0, -2]]
        self.min_dist = 0.12
        ...
        while not valid:
            ...
            for cube_i in range(num_cubes):
                for cube_j in range(num_cubes):
                    if (
                        np.linalg.norm(
                            cubes_qpos[cube_i][:3] - self.initial_cubes_pos[cube_j]
                        )
                        <= self.min_dist
                    ):
                        valid = False
            ...
            if valid:
                for cube_i in range(num_cubes):
                    ...
                    self.initial_cubes_pos[cube_i] = cubes_qpos[cube_i][:3]
